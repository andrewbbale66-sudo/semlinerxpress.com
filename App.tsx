...
import {CurvedArrowDownIcon, Logo} from './components/icons';
...
const App: React.FC = () => {
  ...
  const handleGenerate = useCallback(async (params: GenerateVideoParams) => {
    ...
    try {
      ...
      let userFriendlyMessage = `Shipment visualization failed: ${errorMessage}`;
    ...
    if (lastConfig && lastVideoBlob && lastVideoObject) {
      try {
        ...
        setInitialFormValues({
          ...lastConfig, // Carry over model, aspect ratio, tracking number
          mode: GenerationMode.EXTEND_VIDEO,
          prompt: '', // Start with a blank prompt
          trackingNumber: lastConfig.trackingNumber, // Ensure tracking number persists
  ...
  const renderError = (message: string) => (
    <div className="text-center bg-red-500/10 border border-red-500 p-8 rounded-lg">
      <h2 className="text-2xl font-bold text-red-400 mb-4">Error</h2>
      <p className="text-red-300">{message}</p>
      <button
        onClick={handleTryAgainFromError}
        className="mt-6 px-6 py-2 bg-[var(--sxl-blue)] rounded-lg hover:bg-[var(--sxl-blue)/90] transition-colors">
        Try Again
      </button>
    </div>
  );
  ...
  return (
    <div className="h-screen bg-[var(--sxl-dark-navy)] text-[var(--sxl-light-slate)] flex flex-col overflow-hidden">
      {showApiKeyDialog && (
        <ApiKeyDialog onContinue={handleApiKeyDialogContinue} />
      )}
      <header className="py-4 flex justify-center items-center px-8 relative z-10">
        <div className="flex items-center gap-4">
          <Logo className="w-14 h-14" />
          <h1 className="text-4xl font-bold tracking-tight text-center text-[var(--sxl-lightest-slate)]">
            Semliner Xpress Logistics
          </h1>
        </div>
      </header>
          ...
          <>
            <div className="flex-grow flex items-center justify-center">
              <div className="relative text-center">
                <h2 className="text-3xl text-[var(--sxl-slate)]">
                  Describe your shipment to visualize its journey
                </h2>
                <CurvedArrowDownIcon className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-24 h-24 text-[var(--sxl-light-navy)] opacity-60" />
              </div>
            </div>
            <div className="pb-4">
              <PromptForm
          ...
          <div className="flex-grow flex items-center justify-center">
            ...
            {appState === AppState.SUCCESS && videoUrl && (
              <VideoResult
                ...
                trackingNumber={lastConfig?.trackingNumber}
              />
            )}
            {appState === AppState.SUCCESS &&
              !videoUrl &&
              renderError(
                'Visualization generated, but URL is missing. Please try again.',
              )}
